...
begin
  ...
  Msg := nil;
  try
    utf8FileName := UTF8String(UTF8Encode(FileName));
    iResult := SQLite3_Open(PAnsiChar(utf8FileName), Fdb);

    if iResult <> SQLITE_OK then
      if Assigned(Fdb) then
      begin
        Msg := PAnsiChar(Utf8ToAnsi(Sqlite3_ErrMsg(Fdb)));
...
//    self.ExecSQL('PRAGMA temp_store = MEMORY;');
    ...
    if Assigned(Msg) then
      SQLite3_Free(PAnsiChar(UTF8Encode(Msg)));
  end;
...
begin
  try
    if Sqlite3_Prepare_v2(self.fDB, PAnsiChar(UTF8Encode(SQL)), -1, Stmt, NextSQLStatement) <>
      SQLITE_OK then
...
begin
  ...
  if Sqlite3_Prepare(self.fDB, PAnsiChar(UTF8Encode(SQL)), -1, Stmt, NextSQLStatement) <>
    SQLITE_OK then
    RaiseError('Error executing SQL', SQL)
  else
...
begin
  if Assigned(Query.Statement) then
    Sqlite3_Bind_Text(Query.Statement, Index, PAnsiChar(UTF8Encode(Value)), Length(Value), Pointer(SQLITE_STATIC))
  else
...
begin
  Result := Utf8ToAnsi(SQLite3_Version);
end;

procedure TSQLiteDatabase.AddCustomCollate(name: string;
  xCompare: TCollateXCompare);
begin
  sqlite3_create_collation(fdb, PAnsiChar(UTF8Encode(name)), SQLITE_UTF8, nil, xCompare);
end;

procedure TSQLiteDatabase.AddSystemCollate;
begin
  {$IFDEF WIN32}
  sqlite3_create_collation(fdb, PAnsiChar(UTF8Encode('SYSTEM')), SQLITE_UTF16LE, nil, @SystemCollate);
  {$ENDIF}
end;
...
begin
  try
    ...
    begin
      ...
      i := sqlite3_bind_parameter_index(Stmt, PAnsiChar(UTF8Encode(par.name)));
      if i > 0 then
      begin
        case par.valuetype of
          ...
          SQLITE_TEXT:
            sqlite3_bind_text(Stmt, i, PAnsiChar(UTF8Encode(par.valuedata)),
...
begin
  ...
  try
    ...
    if Sqlite3_Prepare_v2(DB.fDB, PAnsiChar(UTF8Encode(SQL)), -1, Stmt, NextSQLStatement) <> SQLITE_OK then
          ...
          begin
            ...
            //get data types
              ...
              for i := 0 to Pred(fColCount) do
                fCols.Add(AnsiUpperCase(Utf8ToAnsi(Sqlite3_ColumnName(stmt, i))));
              for i := 0 to Pred(fColCount) do
              begin
                new(thisColType);
                DeclaredColType := PAnsiChar(Utf8ToAnsi(Sqlite3_ColumnDeclType(stmt, i)));
              ...
              if (ActualColType = SQLITE_NULL) then
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      new(thisstringvalue);
                      ptrValue := PAnsiChar(Utf8ToAnsi(Sqlite3_ColumnText(stmt, i)));
...
begin
  ...
  self.fColCount := 0;
  self.fSQL := SQL;
  if Sqlite3_Prepare_v2(DB.fDB, PAnsiChar(UTF8Encode(SQL)), -1, fStmt, NextSQLStatement) <> SQLITE_OK then
    DB.RaiseError('Error executing SQL', SQL);
  if (fStmt = nil) then
    DB.RaiseError('Could not prepare SQL statement', SQL);
  DB.DoQuery(SQL);
  ...
  for i := 0 to Pred(fColCount) do
    fCols.Add(AnsiUpperCase(Utf8ToAnsi(Sqlite3_ColumnName(fstmt, i))));
...
begin
  Result := PAnsiChar(Utf8ToAnsi(Sqlite3_ColumnText(fstmt, i))) = nil;
end;
...
begin
  Result := PAnsiChar(Utf8ToAnsi(Sqlite3_ColumnText(fstmt, i)));
end;
